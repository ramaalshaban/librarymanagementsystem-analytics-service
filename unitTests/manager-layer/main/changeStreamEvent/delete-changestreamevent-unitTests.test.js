...
describe("DeleteChangeStreamEvent", () => {
  let ControllerClass;
  let req;
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/ChangeStreamEvent/delete-changestreamevent",
      {
        "./ChangeStreamEventManager": class {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("deleteChangeStreamEvent");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("delete");
    expect(instance.dataName).to.equal("changeStreamEvent");
  });
});
