...
describe("UpdateAnalyticSnapshot", () => {
  let ControllerClass;
  let req;
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/AnalyticSnapshot/update-analyticsnapshot",
      {
        "./AnalyticSnapshotManager": class {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("updateAnalyticSnapshot");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("update");
    expect(instance.dataName).to.equal("analyticSnapshot");
  });
});
