...
class DbListChangestreameventsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListChangestreamevents";
    this.emptyResult = true;
    this.objectName = "changeStreamEvents";
  ...
  async transposeResult() {
    for (const changeStreamEvent of this.dbData.items) {
  ...
  createQuery() {
    ...
    return ChangeStreamEvent.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (ChangeStreamEvent.getCqrsJoins) {
      await ChangeStreamEvent.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await ChangeStreamEvent.countDocuments(
      this.whereClause,
    );
  }
...
const dbListChangestreamevents = (input) => {
  const dbGetListCommand = new DbListChangestreameventsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListChangestreamevents;
