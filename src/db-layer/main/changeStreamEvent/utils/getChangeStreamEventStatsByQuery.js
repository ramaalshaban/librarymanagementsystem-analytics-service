...
const getChangeStreamEventStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(ChangeStreamEvent.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            ChangeStreamEvent.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await ChangeStreamEvent.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingChangeStreamEventStatsByQuery",
      err,
    );
  }
};

module.exports = getChangeStreamEventStatsByQuery;
