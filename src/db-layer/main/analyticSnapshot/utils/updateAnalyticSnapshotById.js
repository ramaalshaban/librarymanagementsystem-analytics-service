...
const updateAnalyticSnapshotById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await AnalyticSnapshot.findOne({
      _id: id,
      isActive: true,
    });
    ...
    const dbDoc = await AnalyticSnapshot.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingAnalyticSnapshotById",
      err,
    );
  }
};

module.exports = updateAnalyticSnapshotById;
