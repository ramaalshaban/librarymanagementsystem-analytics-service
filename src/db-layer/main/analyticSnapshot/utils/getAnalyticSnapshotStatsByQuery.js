...
const getAnalyticSnapshotStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(AnalyticSnapshot.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            AnalyticSnapshot.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await AnalyticSnapshot.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingAnalyticSnapshotStatsByQuery",
      err,
    );
  }
};

module.exports = getAnalyticSnapshotStatsByQuery;
