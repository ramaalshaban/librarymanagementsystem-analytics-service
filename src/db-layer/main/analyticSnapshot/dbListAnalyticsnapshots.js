...
class DbListAnalyticsnapshotsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListAnalyticsnapshots";
    this.emptyResult = true;
    this.objectName = "analyticSnapshots";
  ...
  async transposeResult() {
    for (const analyticSnapshot of this.dbData.items) {
  ...
  createQuery() {
    ...
    return AnalyticSnapshot.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (AnalyticSnapshot.getCqrsJoins) {
      await AnalyticSnapshot.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await AnalyticSnapshot.countDocuments(
      this.whereClause,
    );
  }
...
const dbListAnalyticsnapshots = (input) => {
  const dbGetListCommand = new DbListAnalyticsnapshotsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListAnalyticsnapshots;
