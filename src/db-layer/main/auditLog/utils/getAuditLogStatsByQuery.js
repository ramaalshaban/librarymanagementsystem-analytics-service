...
const getAuditLogStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(AuditLog.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            AuditLog.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await AuditLog.countDocuments(queryWithSoftDelete);
...
module.exports = getAuditLogStatsByQuery;
