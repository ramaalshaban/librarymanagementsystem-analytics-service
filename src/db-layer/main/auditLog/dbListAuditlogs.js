...
class DbListAuditlogsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
  ...
  createQuery() {
    ...
    return AuditLog.find(this.whereClause);
  }
  ...
  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await AuditLog.countDocuments(
      this.whereClause,
    );
  }
