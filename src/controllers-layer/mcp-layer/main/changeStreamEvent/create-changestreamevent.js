...
class CreateChangeStreamEventMcpController extends AnalyticsMcpController {
  constructor(params) {
    super("createChangeStreamEvent", "createchangestreamevent", params);
    this.dataName = "changeStreamEvent";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateChangeStreamEventManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Create a new MongoDB change stream event entry (for dev/test/visualization).",
    parameters: CreateChangeStreamEventMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const createChangeStreamEventMcpController =
        new CreateChangeStreamEventMcpController(mcpParams);
      try {
        const result =
          await createChangeStreamEventMcpController.processRequest();
        //return CreateChangeStreamEventMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
