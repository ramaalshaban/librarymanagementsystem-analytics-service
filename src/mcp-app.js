...
app.post("/mcp", async (req, res) => {
  ...
  if (sessionIdHeader && sessions[sessionIdHeader]) {
    ...
    const {
      ...
      analyticSnapshotMcpRouter,
      auditLogMcpRouter,
      changeStreamEventMcpRouter,
      getSessionRouter,
    } = require("mcpLayer")(initialHeaders);

    const sessionRouter = getSessionRouter(initialHeaders);

    analyticSnapshotMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    auditLogMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    changeStreamEventMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
